...
class ImageHelper{
  ...
  handleResizeImage(fileName, pathFile, destFile, widths) {
    return new Promise(function(resolve, reject) {
  ...
  resizeImage(file, width, height, pathFile) {
  ...
  resizeImageProcess(source, dest, widths) {
    //console.log('Starting resize images');
    
    self.createDir(source)
      .then(function() {
        return self.createDir(dest);
      })
      .then(function() {
        return self.getAllFiles(source);
      })
      .then(function(files) {
        var promises = [];
        for (var i in files) {
          ...
          var promise = self.handleResizeImage(file, pathFile, dest,widths);
          promises.push(promise);
        }
